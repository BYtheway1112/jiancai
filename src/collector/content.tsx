...
import { Platform,Target,assertSamePost,postType } from './model';
import {Context,collect,context,authorPosts} from './platform';
import {rpc} from './rpc';
...
type PanelKind = 'sync'|'batch'|'eagle'|'download'|null;
...
function Toolbar({c}:{c:Context}){
 ...
 const open=async(kind:'sync'|'batch'|'eagle')=>{setPanel(kind);setMessage('');try{if(kind==='eagle'){setEagle(await rpc('eagleInfo'));return;}const list=await rpc<Target[]>('targets');const same=list.filter(t=>t.platform===c.platform);setTargets(same);setTarget(target&&same.some(t=>t.id===target)?target:same[0]?.id||'');}catch(e){setMessage((e as Error).message);}};
 ...
 return <><div className="bar" aria-label="简采工具栏" data-build="20260923.02">{c.kind==='post'?<>{features.download&&<button disabled={busy} onClick={()=>execute(download,'download')}>↓ 下载</button>}{features.feishu&&<button disabled={busy} onClick={()=>target?execute(sync):open('sync')}><ServiceLogo name="feishu" tone="white"/>飞书</button>}{features.eagle&&<button disabled={busy} onClick={()=>eagle?execute(importToEagle):open('eagle')}><ServiceLogo name="eagle" tone="white"/>Eagle</button>}{features.more&&<details><summary>更多 ···</summary><div className="menu"><button onClick={()=>open('sync')}>选择飞书目标表</button><button onClick={()=>open('eagle')}>选择 Eagle 文件夹</button><button onClick={()=>execute(()=>more('copy'))}>复制信息</button><button onClick={()=>execute(()=>more('post-comment'))}>导出评论</button>{c.platform==='dy'&&<button onClick={()=>execute(()=>more('mix-post'))}>导出合集</button>}<button onClick={()=>rpc('openOptions')}>采集列表 / 设置</button></div></details>}</>:<><button disabled={busy} onClick={()=>open('batch')}>＋ 采集主页作品</button><button onClick={()=>rpc('openOptions')}>采集列表</button></>}</div>
 {panel&&<section className="panel" aria-label="简采面板"><button className="close" disabled={busy} onClick={()=>setPanel(null)}>关闭</button><h3>{panel==='batch'?'采集主页作品':panel==='eagle'?'导入 Eagle':panel==='download'?'下载图文':'保存作品'}</h3>
 ...
 {panel!=='download'&&<div className="actions"><button className="primary" disabled={busy} onClick={()=>execute(panel==='batch'?batch:panel==='eagle'?importToEagle:sync)}>{panel==='batch'?'开始采集':panel==='eagle'?'导入并验证':'同步这条作品'}</button>{busy&&panel==='batch'&&<button onClick={()=>stop.current?.abort()}>停止</button>}<button onClick={()=>rpc('openOptions')}>打开采集列表 / 设置</button></div>}<div role="status" className="message">{message}</div></section>}
 </>;
}
