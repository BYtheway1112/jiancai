...
function Toolbar({c}:{c:Context}){
 ...
 const mixSync=async()=>{if(c.platform!=='dy')throw new Error('合集采集仅支持抖音');if(!mixInfo?.mix_id)throw new Error('没有取得合集信息');if(!target)throw new Error('请先选择抖音视频目标表');const controller=new AbortController();stop.current=controller;let success=0,failed=0,partial=0;const errors:string[]=[];try{for await(const raw of mixPosts(String(mixInfo.mix_id),controller.signal)){if(controller.signal.aborted)break;assertSamePost(c,context(c.platform));const id=String(raw.aweme_id);const href=raw.share_url||`https://www.douyin.com/video/${id}`;try{
   // The mix endpoint may omit mix_info on individual entries. Carry the
   // collection identity into normalization so the Feishu "所属合集"
   // field remains populated for every record, including image posts.
   const mixIdentity={mix_id:String(mixInfo.mix_id),mix_name:mixInfo.mix_name};
   const entry={...raw,mix_info:{...mixIdentity,...(raw.mix_info||{})}};
   let post=normalize('dy',entry,href,raw.author);
   if(!post.media.some(m=>m.field==='视频文件'||m.field==='视频图片')){
    post=await collect({platform:'dy',id,href,kind:'post'},raw.author);
    if(!post.fields['所属合集']&&mixInfo.mix_name)post={...post,fields:{...post.fields,'所属合集':mixInfo.mix_name}};
   }
   if(controller.signal.aborted)break;await rpc('savePost',post);const result=await rpc('sync',{key:post.key,targetId:target,useDefault:false});success++;if(result.warnings.length){partial++;errors.push(`${id}：${result.warnings.join('；')}`);}}catch(e){failed++;errors.push(`${id}：${(e as Error).message}`);if(/登录|验证|频繁|风控|captcha|游标/i.test((e as Error).message))throw e;}const progress=`已处理 ${success+failed} 条：成功 ${success}，失败 ${failed}，部分素材 ${partial}`;setMixProgress(progress);setMessage(`${progress}\n${errors.slice(-3).join('\n')}`);await new Promise(r=>setTimeout(r,800));}}catch(e){errors.push('采集终止：'+(e as Error).message);}finally{setMixProgress('');setMessage(`${controller.signal.aborted?'已停止':'合集采集结束'}：成功 ${success} 条，失败 ${failed} 条，部分素材 ${partial} 条。${errors.length?'\n'+errors.join('\n'):''}`);}};
 ...
 {panel==='mix'&&<><p className="muted">合集：{mixInfo?.mix_name||'未命名合集'}，预计 {mixInfo?.statis?.updated_to_episode??'未知'} 条。按合集顺序串行采集，已完成记录会保留。</p>{mixProgress&&<p className="progress" role="status">{mixProgress}</p>}<label>飞书目标表</label><select disabled={busy} value={target} onChange={e=>{setTarget(e.target.value);setTargetPicked(true);}}><option value="">选择抖音目标表</option>{targets.map(t=><option key={t.id} value={t.id}>{t.name}</option>)}</select><p className="muted">图文和视频会按作品类型自动路由到对应抖音表；重复作品更新采集字段，人工字段保留。</p></>}
